...
test('generate sends a style-specific prompt', function () {
...
test('generate maps a portrait orientation to a 9:16 aspect ratio', function () {
    (new AiImageClient)->generate(['x'], ImageStyle::Cinematic, orientation: 'portrait');
    Http::assertSent(fn ($r) => $r['aspect_ratio'] === '9:16');
});

test('generate maps a landscape orientation to a 16:9 aspect ratio', function () {
    (new AiImageClient)->generate(['x'], ImageStyle::Cinematic, orientation: 'landscape');
    Http::assertSent(fn ($r) => $r['aspect_ratio'] === '16:9');
});

test('generate falls back to a 1:1 aspect ratio for an unknown orientation', function () {
    (new AiImageClient)->generate(['x'], ImageStyle::Cinematic, orientation: 'whatever');
    Http::assertSent(fn ($r) => $r['aspect_ratio'] === '1:1');
});

test('generate appends the Arabic instruction when language is ar', function () {
    (new AiImageClient)->generate(['x'], ImageStyle::Cinematic, language: 'ar');
    Http::assertSent(fn ($r) => str_contains((string) $r['prompt'], 'Arabic'));
});

test('generate defaults to English when the language is unsupported', function () {
    (new AiImageClient)->generate(['x'], ImageStyle::Cinematic, language: 'sv');
    Http::assertSent(fn ($r) => str_contains((string) $r['prompt'], 'English'));
});

test('generate appends the brand palette when colours are provided', function () {
    (new AiImageClient)->generate(['x'], ImageStyle::Infographic, brandColor: '#facc15', backgroundColor: '#ffffff', textColor: '#0f172a');
    Http::assertSent(fn ($r) => str_contains((string) $r['prompt'], 'BRAND COLOR PALETTE')
...
test('generate omits the brand palette when no colours are set', function () {
...
test('generate appends brand context when a brand description is provided', function () {
    (new AiImageClient)->generate(['x'], ImageStyle::Cinematic, brandDescription: 'a fitness coaching brand for busy professionals');
    Http::assertSent(fn ($r) => str_contains((string) $r['prompt'], 'Brand context')
        && str_contains((string) $r['prompt'], 'fitness coaching'));
});

test('generate truncates a brand description longer than 200 chars', function () {
...
test('generate omits brand context when the description is only whitespace', function () {
